...
namespace TreeListViewDragDrop {
    public partial class Form1 : Form {
        public string host = "localhost"; // TODO: Initialize to an appropriate value
        ...
        private void Form1_Load(object sender, EventArgs e) {
            ...
            try
            {
                ...
                if (this.LocalPathRoot == null || this.LocalPathRoot == "") this.LocalPathRoot = @"C:\";

                string[] aFiles = null;
                if (sKeyFiles != null)
                    aFiles = sKeyFiles.Split('|');
                    ...
                    if (FtpMode == 0)
                    {
                        if (keyFiles != null)
                        {
                            if (keyFiles.Length > 0)
                            {
                                ...
                                this.ftpSessionGen.sftpClient = new SftpClient(this.host, port, username, keyFiles);
                            }
                            else
                                ftpSessionGen = new FtpSessionGen(FtpMode, host, username, password, port);
                        }
                ...
                else
                {
                    ...
                    treeListView1.EmptyListMsg = "Host: " + host + " - connection failed " + strMessError;
                }
        ...
        private void btnCreateDirectory_Click(object sender, EventArgs e)
        {
            ...
            FormCreateDir frmNewdir = new FormCreateDir();
            if (frmNewdir.ShowDialog() == DialogResult.OK)
                if (frmNewdir.NewDir != "")
                    dirname = frmNewdir.NewDir;
            ...
            newdir = (model1.RemotePath == "" || model1.RemotePath == @"\" || model1.RemotePath == @"/") ? dirname : model1.RemotePath + "/" + dirname;
        ...
        {
            FormConnection frmCon = new FormConnection();
            frmCon.Parent = this;
            frmCon.ShowDialog();
            if (frmCon.ConnectionIsOk)
            {
                FtpMode = frmCon.FtpMode;
                host = frmCon.host;
                port = frmCon.port;
                username = frmCon.username;
                password = frmCon.password;

                keyFiles = frmCon.keyFiles;
                aKeyFiles = frmCon.aKeyFiles;
                sKeyFiles = frmCon.sKeyFiles;

                if (ftpSessionGen != null)
                    ftpSessionGen.Disconnect();

                if (FtpMode == 0 && keyFiles != null)
                {
                    if (keyFiles.Length > 0)
                    {
                        ftpSessionGen = new FtpSessionGen(FtpMode, host, username, password, port);
                        this.ftpSessionGen.sftpClient = new SftpClient(host, port, username, keyFiles);
                    }
                    else
                        ftpSessionGen = new FtpSessionGen(FtpMode, host, username, password, port);
                }
                ...
                ftpSessionGen.Connect();
                if (ftpSessionGen.IsConnected()) IsConnected = true;
                            ...
                            foreach (FtpFileClass f in files)
                                if (f.IsDirectory && f.Name != "." && f.Name != "..")
                                {
                                    ...
                                    models.Add(new ModelWithChildren
                                    {
                                        ...
                                        RemotePath = path,
                                        Parent = parent,
                                        Now = f.LastWriteTime.ToShortDateString() + " " + f.LastWriteTime.ToShortTimeString(),
                        ...
                        if (parent.LocalRemote == 1)
                        {
                            ...
                            foreach (string dir in dirs)
                            {
                                ...
                                models.Add(new ModelWithChildren
                                {
                                    ...
                                    LocalRemote = 1,
                                    Update = "dir",
                                    Now = datetimedir.ToShortDateString() + " " + datetimedir.ToShortTimeString(),
                            ...
                            if (parent.LocalPath != null)
                            {
                                ...
                                foreach (string x in filesindir)
                                {
                                    ...
                                    models.Add(new ModelWithChildren
                                    {
                                        ...
                                        LocalRemote = 1,
                                        Update = "file",
                                        Now = datetimefile.ToShortDateString() + " " + datetimefile.ToShortTimeString(),
