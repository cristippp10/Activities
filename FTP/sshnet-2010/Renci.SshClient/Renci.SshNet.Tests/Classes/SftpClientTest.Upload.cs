...
namespace Renci.SshNet.Tests.Classes
{
    ...
    public partial class SftpClientTest : TestBase
    {
        ...
        [TestMethod]
        [TestCategory("Sftp")]
        public void Sftp_Multiple_Async_Download_Files(SftpClient sftp,  string localPath, string remotePath, string pattern)
        {
            var maxFiles = 10;
            var maxSize = 5;
            ...
            //    sftp.Connect();
               ...
               var testInfoList = new Dictionary<string, TestInfo>();
                ...
                foreach (SftpFile remoteFile in files) //in testInfoList.Keys)
                {
                    //var testInfo = remoteFile.IsRegularFile;//testInfoList[remoteFile];
                    if (remoteFile.IsRegularFile)
                    {
                        string fullname = remoteFile.FullName;
                        string filename = remoteFile.Name;

                        var testInfo = new TestInfo();
                        testInfo.UploadedFileName = fullname;
                        testInfo.DownloadedFileName = localPath + "\\" + filename;
                        testInfo.RemoteFileName = fullname;
                    ...
                    { 
                        if (remoteFile.IsDirectory)
                        {
                            ...
                            {
                                if (!Directory.Exists(localPath + "\\" + filename)) 
                                    Directory.CreateDirectory(localPath + "\\" + filename);
                                Sftp_Multiple_Async_Download_Files(sftp, localPath + "\\" + filename, remotePath + "/" + filename, pattern);
                            }
                        }
                    }
