...
namespace TreeListViewDragDrop
{
    public partial class FormConnection : Form
    {
        ...
        private FtpSessionGen ftpSessionGen;
        ...
        private void FormConnection_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (FtpMode == 0)
                {
                    ...
                    groupBoxKeys.Visible = true;
                }
                else
                {
                    chkSftp.Checked = false;
                    chkFtp.Checked = true;
                    groupBoxKeys.Visible = false;
                }
        ...
        void VerifyConnection()
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!chkFtp.Checked)
                    {
                        ...
                        if (keyFiles != null)
                        {
                            if (keyFiles.Length > 0)
                            {
                                ...
                                ftpSessionGen.sftpClient = new SftpClient(this.host, port, username, keyFiles);
                            }
                            else
                                ftpSessionGen = new FtpSessionGen(FtpMode, host, username, password, port);
                        }
                    ...
                    {
                        FtpMode = 1;
                        ftpSessionGen = new FtpSessionGen(1, host, username, password, port);
                    }
                    ...
                    if (ConnectionIsOk) ftpSessionGen.Disconnect();
                }
                catch (Exception ex)
                {
                    sExMess = ex.Message;
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Parent.ftpSessionGen = ftpSessionGen;
                Parent.host = host;
                Parent.port = port;
                Parent.password = password;
                Parent.username = username;
                Parent.FtpMode = FtpMode;
                //Parent.ftpSessionGen = 
                Parent.sKeyFiles = sKeyFiles;
                Parent.keyFiles = keyFiles;
            }
            if (this.ConnectionIsOk)
                Close();
        }
        ...
        {
            if (chkSftp.Checked)
                { chkFtp.Checked = false; if (chkKeys.Checked) { groupBoxKeys.Enabled = true; } chkKeys.Enabled = true; groupBoxKeys.Visible = true; }
            else
        ...
        private void chkFtp_CheckedChanged(object sender, EventArgs e)
        {
            if (chkFtp.Checked)
                ...
                { chkSftp.Checked = true; if (chkKeys.Checked) { groupBoxKeys.Enabled = true; } chkKeys.Enabled = true; groupBoxKeys.Visible = true; }
        }
