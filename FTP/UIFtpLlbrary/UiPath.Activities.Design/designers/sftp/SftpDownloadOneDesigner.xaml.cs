...
{
	public partial class SftpDownloadOneDesigner : ActivityDesigner, IComponentConnector
	{
        ...
        public SftpDownloadOneDesigner()
		{
		...
		private void LoadButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (modelitemparentup.Parent != null)
                if (modelitemparent.Parent != null)
                    if (modelitemparent.Properties != null)  
            {
                //InArgument<string> Host = (InArgument<string>)base.ModelItem.Parent.Parent.Parent.Properties["Host"].Value.GetCurrentValue();
                //string host = Host.Expression.ToString();
                ...
                if (dlgResult == DialogResult.OK)
                {
                    ...
                    if (alocal != null)
                        if (alocal.Count == 1)
                        {
                            string pathdownload = ((TreeListViewDragDrop.Form1.ModelWithChildren)alocal[0]).LocalPath;
                            base.ModelItem.Properties["LocalPath"].SetValue(new InArgument<string>(pathdownload));
                        }
                    //foreach (object mo in alocal)
                    //{
                    //    TreeListViewDragDrop.ModelWithChildren mod = (TreeListViewDragDrop.ModelWithChildren)mo;
                    ...
                    if (strRemoteFiles == "" || strRemoteFiles == "|")
                        System.Windows.Forms.MessageBox.Show("remote path not selected!");
                    else
                    {
                        if (strRemoteFiles[strRemoteFiles.Length - 1] == '|') strRemoteFiles = strRemoteFiles.Substring(0, strRemoteFiles.Length - 1);
                        base.ModelItem.Properties["RemotePath"].SetValue(new InArgument<string>(strRemoteFiles));
                    }
                    //base.ModelItem.Properties["ListLocal"].SetValue(new InArgument<string>(strLocalFiles));
                    //base.ModelItem.Properties["ListRemote"].SetValue(new InArgument<string>(strRemoteFiles));
