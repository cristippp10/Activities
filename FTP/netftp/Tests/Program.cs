...
namespace Tests {
    ...
    class Program {
        ...
        static void Upload(FtpClient cl, string localpath, string remotepath) {
        ...
        static void DoUpload(FtpClient cl, string remotepath, string s) {
        ...
        static void Download(List<Thread> threads, FtpClient cl, string path, string localpath) {
            if (localpath.Length > 0)
                if (localpath[localpath.Length - 1] != '\\')
                    localpath = localpath + @"\";
            foreach (FtpListItem item in cl.GetListing(path)) {
                if (item.Type == FtpFileSystemObjectType.Directory)
                    Download(threads, cl, item.FullName, localpath);
        ...
        static void DoDownload(FtpClient cl, string file, string localpath) {
